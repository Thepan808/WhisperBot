...
async def _callbacks(bot, callback_query: CallbackQuery):
	...
	if callback_query.data.lower() == "home":
		...
		if callback_query.from_user.id in data_list:
			...
			await callback_query.answer("Desculpe baianor, você não pode ver esta mensagem privada, porque não é para você... Se fosse, você não perderia seu tempo vindo aqui. Deixe de curiosidade e vai lavar uma louça 🤡!", show_alert=True)
